...
const LandingPage: React.FC<LandingPageProps> = ({ onStartChat }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (uploadedFiles.length > 0) {
        ...
        for (const uploadedFile of uploadedFiles) {
          const fileType =
            ...
            uploadedFile.file.name.split(".").pop()?.toLowerCase() ||
            "txt";
      ...
      const botText =
        data.response || data.output || "No response from agent";

      setMessages((prev) =>
        prev.map((msg) =>
          msg.isTyping && !msg.isUser
            ? { ...msg, text: botText, isTyping: false, showTypewriter: true }
            : msg
        )
      );
    } catch (err) {
  ...
  return (
    <div className="flex h-screen bg-background relative">
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 sidebar-container sidebar-transition ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <ElevenDocsSidebar onLogout={logout} onNewChat={handleNewChat} />
      </div>
      ...
      {/* Main Content */}
      <div
        className={`flex-1 flex flex-col main-content-transition ${
              ...
              <div className="flex-1 overflow-y-auto bg-chat-bg chat-scroll-area">
                <div className="max-w-3xl mx-auto px-4">
                  {messages.map((message, index) => (
                    <div key={message.id} className={`my-2 flex ${message.isUser ? "justify-end" : "justify-start"}`}>
                      <span className={`inline-block px-4 py-2 rounded-lg ${message.isUser ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-900"}`}>
                        {message.text}
                      </span>
                    </div>
                  ))}
